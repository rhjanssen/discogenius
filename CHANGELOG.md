...
- **Metadata sidecar writers now fall back to canonical/provider-item metadata:** `metadata-files` no longer uses `ProviderAlbums`/`ProviderMedia` for local album/video NFO or artwork fallback data. Artist NFO album lists read canonical `Albums`; album NFOs resolve MBIDs, selected releases, reviews, artwork context, and tracks through `ProviderItems`, `ReleaseGroupSlots`, `AlbumReleases`, and `Tracks`; video NFOs resolve artist/album MBIDs through video `ProviderItems` + canonical `Recordings`. Existing lyric-service fallback remains isolated for the later lyric cutover.
