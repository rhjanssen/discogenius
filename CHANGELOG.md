...
- **Upgrader scan is canonical/provider-item first:** `CheckUpgrades` now reads installed files from `TrackFiles` canonical/provider identity plus `ProviderItems`, not `ProviderMedia`/`ProviderAlbums`. Canonical-only audio and video files can queue upgrade downloads with zero legacy provider rows; the transitional `upgrade_queue` ledger remains keyed to legacy `media_id`/`album_id` until the Phase 5 schema re-key.
