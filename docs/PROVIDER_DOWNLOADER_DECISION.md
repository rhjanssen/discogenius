...
| Amazon Music | [`amazon-music`](https://github.com/AmineSoukara/Amazon-Music) | Direct entitled Amazon audio through an unofficial API/session, including lossless/hi-res/Atmos when exposed | Keep capability-gated and describe it honestly as unofficial. The official Amazon Music Web API is not a general public download API. |
| Spotify | [`Votify`](https://github.com/glomatico/votify) with librespot | Direct Spotify Vorbis audio for a Premium session; no lossless, spatial or video claim | Keep as the optional direct path. [`spotDL`](https://github.com/spotDL/spotify-downloader) reads Spotify metadata/lists but searches and downloads the audio from YouTube, so using it under the Spotify provider would misstate provenance and duplicate Discogenius's separate YouTube provider. |
