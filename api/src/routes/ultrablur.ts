...
function renderUltraBlurImage(colors: UltraBlurColors, width: number, height: number): Buffer {
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      // Faint dither to break up gradient banding. Kept small: larger amplitudes
      // read as visible grain once the image is upscaled, especially in dark
      // regions (noise is added in linear light, where the sRGB curve is steep).
