...
function initializeAuthEnvironment() {
  ...
  // Playback URLs use the same HMAC key even when the optional dashboard
  // password is disabled. Never let an unauthenticated install fall back to a
  // public, hard-coded stream signing secret.
...
const hostname = String(process.env.DISCOGENIUS_BIND_IP || "127.0.0.1").trim() || "127.0.0.1";
...
const configuredCorsOrigins = new Set(
  String(process.env.DISCOGENIUS_CORS_ORIGINS || "")
    .split(",")
...
app.use(cors((req, callback) => {
  ...
  let allowed = !origin || configuredCorsOrigins.has(origin);
  if (origin && !allowed) {
    try {
      const requestOrigin = `${req.protocol}://${req.get("host")}`;
      allowed = new URL(origin).origin === requestOrigin;
    } catch {
      allowed = false;
    }
  }
...
app.use("/api/v1/provider", authMiddleware, providerRouter);
...
app.use("/api/v1/system/status", authMiddleware, systemStatusRouter);
app.use("/api/v1/mediaFile", authMiddleware, mediaFileRouter);
...
// Docker / Compose liveness. Public health responses deliberately expose only
// aggregate status; authenticated diagnostics live on /api/health. A failed
// deep check or last week's import must not mark the container unhealthy and
// block the Apple Music wrapper's depends_on.
