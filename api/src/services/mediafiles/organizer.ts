  ...
  public static async organizeDownload(raw: OrganizeRequest): Promise<OrganizeResult> {
    ...
    if (type === "album") {
      ...
      for (const srcFile of files) {
        ...
        const renderedTrackPath = renderRelativePath(trackTemplate, {
          ...
          releaseGroupMbId: canonicalAlbum?.releaseGroupMbid || album.mbid || null,
          ...
          editionMbId: canonicalAlbum?.editionMbid || jobReleaseMbid || null,
          releaseMbId: canonicalAlbum?.editionMbid || jobReleaseMbid || null,
          albumGenre: firstGenre(canonicalAlbum?.genres),
          albumYear: getReleaseYear(canonicalAlbum?.albumReleaseDate || album.release_date),
          editionYear: getReleaseYear(canonicalAlbum?.editionReleaseDate || canonicalAlbum?.releaseDate),
    ...
    if (type === "track") {
      ...
      const renderedTrackPath = renderRelativePath(trackTemplate, {
        ...
        editionTitle: canonicalAlbum?.editionTitle || canonicalAlbum?.title || "Unknown Album",
        editionDisambiguation: canonicalAlbum?.editionDisambiguation || null,
        editionMbId: canonicalAlbum?.editionMbid || trackIdentity.canonicalReleaseMbid || null,
        releaseMbId: canonicalAlbum?.editionMbid || trackIdentity.canonicalReleaseMbid || null,
        albumGenre: firstGenre(canonicalAlbum?.genres),
        albumYear: getReleaseYear(canonicalAlbum?.albumReleaseDate || album.release_date),
        editionYear: getReleaseYear(canonicalAlbum?.editionReleaseDate || canonicalAlbum?.releaseDate),
        releaseYear: getReleaseYear(canonicalAlbum?.releaseDate || album.release_date),
