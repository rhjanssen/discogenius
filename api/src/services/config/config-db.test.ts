...
test("fresh installs use the production monitoring and library defaults", () => {
  ...
  assert.equal(config.naming.album_track_path_single, "{Edition Title} ({Release Year}) {mbid-{Edition MbId}}/{track:00} - {Track Title}");
  assert.equal(config.naming.album_track_path_multi, "{Edition Title} ({Release Year}) {mbid-{Edition MbId}}/{medium:0}{track:00} - {Track Title}");
