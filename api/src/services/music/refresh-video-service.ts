    ...
    new ProviderMatchRepository(db).upsertVideoMatch({
        ...
        decision: {
            ...
            evidence: {
                ...
                relatedTrackId: nullableText(
                    input.video.related_track_id ?? input.video.relatedTrackId,
                ),
...
function findAudioRecordingByProviderTrack(
    ...
    // A provider track can occur on several provider editions. It identifies an
    // audio recording only when every accepted occurrence agrees; choosing the
    // first row would invent edition context.
    const recordingIds = new Set(rows.map((row) => Number(row.id)));
    if (recordingIds.size !== 1) {
        return { match: null, acceptedOccurrenceCount: rows.length };
    }
    const row = rows[0];

    // Provider relationships are strong evidence, not permission to collapse
    // different cuts. Apple can relate a live video to a studio/reprise song;
    ...
    const compatible = findRelatedAudioRecordingForVideo(video, [candidate], {
        videoVariant: parseVideoVariant(nullableText(video.title)),
    ...
    return { match: {
        id: compatible.id,
        mbid: compatible.mbid,
        confidence: 0.96,
        method: "provider-video-related-track",
        evidence: {
            relatedTrackId: providerTrackId,
            audioTitle: candidate.title ?? null,
            validatedBy: compatible.method,
        },
...
function resolveProviderVideoAudioMatch(input: {
    ...
    if (relatedTrackId) {
        ...
        if (byTrack.acceptedOccurrenceCount > 0) return null;
...
function ensureProviderVideoRecording(input: {
    ...
    const sameProviderExclude = provider && providerId
        ? { provider, providerId, isrc: nullableText(input.video.isrc) }
        : null;
    ...
    const finish = (recordingId: number): number => {
        ...
        if (input.existingRecordingId && input.existingRecordingId !== survivor) {
            ...
            // Fold only obsolete provider-only shells. A YouTube watch id is a
            // catalog identity too, so coalescing a newly split live/alternate
            // cut back into that row would immediately undo the repair.
            if (
                existing
                && !existing.mbid
                ...
                && provider
                && providerId
                && !recordingHasOtherAcceptedVideoOffer(existing.id, provider, providerId)
            ) {
    ...
    if (input.existingRecordingId) {
        ...
        const existing = db.prepare(`
            SELECT id, title, length_ms, mbid, youtube_video_id, video_variant, release_date
            FROM Recordings
            WHERE id = ? AND is_video = 1
        `).get(input.existingRecordingId) as {
            id: number;
        ...
        const sameProviderSiblings = existing && sameProviderExclude
            ? otherOffersFromProvider(
                existing.id,
                sameProviderExclude.provider,
                sameProviderExclude.providerId,
            )
            : [];
        const exactCanonicalFit = Boolean(
            existing
            && (existing.mbid || existing.youtube_video_id)
            && offerIsExactTwinOfRecording(rawTitle, lengthMs, releaseDate, existing),
        );
        const sameProviderCollision = sameProviderSiblings.length > 0
            && !sameProviderOffersAreExactTwins(
                rawTitle,
                lengthMs,
                releaseDate,
                sameProviderExclude?.isrc,
                sameProviderSiblings,
            )
            && !exactCanonicalFit;
...
function applyCatalogVideoIdentity(
    ...
    const existing = db.prepare(`
        SELECT title, video_variant, mbid, youtube_video_id
        FROM Recordings
        WHERE id = ?
    `).get(recordingId) as {
        title: string | null;
    ...
    const existingVariant = normalizeVideoVariant(existing.video_variant);
    const refinedVariant = preferredVideoVariant(existingVariant, input.offerVariant);
    const refineProviderOnlyIdentity = !existing.mbid
        && !existing.youtube_video_id
        && refinedVariant !== existingVariant;
    ...
    `).run(
        ...
        input.groupTitle,
        refineProviderOnlyIdentity ? 1 : 0,
        input.groupTitle,
 ...
 */
function otherOffersFromProvider(
    recordingId: number,
    provider: string,
    providerId: string,
): Array<{
    ...
    return db.prepare(`
        SELECT item.title, item.duration_ms, item.release_date, item.isrc
    ...
    `).all(recordingId, provider, providerId) as Array<{
        title: string | null;
        duration_ms: number | null;
        release_date: string | null;
        isrc: string | null;
...
/**
 * Provider-native facts already accepted into a recording cluster. The
 * Recordings row is a canonical summary and can lag a later provider detail
 * refresh, so it must not be the only duration/date evidence used to attach a
 * new offer.
 */
function authoritativeAcceptedVideoOffers(recording: {
    id: number;
    mbid: string | null;
    youtube_video_id: string | null;
    ...
    if (recording.youtube_video_id) {
        return offers.filter((offer) =>
            ...
            && String(offer.provider_id) === String(recording.youtube_video_id));
    }
    if (recording.mbid) {
        return offers.filter((offer) =>
            offer.method === "musicbrainz-recording-url"
            || offer.method === "external_id");
    }
    return [];
}

function recordingHasOtherAcceptedVideoOffer(
    recordingId: number,
    provider: string,
    providerId: string,
): boolean {
...
function sameProviderOffersAreExactTwins(
    title: string,
    lengthMs: number | null,
    releaseDate: string | null | undefined,
    isrc: string | null | undefined,
    siblings: ReturnType<typeof otherOffersFromProvider>,
): boolean {
    return siblings.every((sibling) => isExactVideoTwin({
        titleA: title,
        titleB: String(sibling.title || ""),
        lengthMsA: lengthMs,
        lengthMsB: sibling.duration_ms,
        variantA: parseVideoVariant(title),
        variantB: parseVideoVariant(sibling.title),
        releaseDateA: releaseDate,
        releaseDateB: sibling.release_date,
        isrcsA: isrc,
        isrcsB: sibling.isrc,
    }));
}

function offerIsExactTwinOfRecording(
    title: string,
    lengthMs: number | null,
    releaseDate: string | null | undefined,
    recording: {
...
function findVideoRecordingIdByTitle(
    ...
    const isEligibleCandidate = (row: (typeof rows)[number]): boolean => {
        ...
        if (
            (row.mbid || row.youtube_video_id)
            && offerIsExactTwinOfRecording(title, lengthMs, releaseDate, row)
        ) {
            return true;
        }
        // Compare provider-native offer facts. Recording aggregates can carry a
        // different provider's title/duration after an earlier merge, so using
        // them here makes same-provider collision handling order-dependent.
    ...
    const scored = rows
        ...
        .map((row) => {
            ...
            const candidateSignals = [
                {
                    title: rowTitle,
                    durationMs: row.length_ms,
                    releaseDate: row.release_date,
                    isrc: null,
                    variant: rowVariant,
                    provider: peerProvider(row),
                },
                ...canonicalOffers(row).map((offer) => ({
                    title: String(offer.title || rowTitle),
                    durationMs: offer.duration_ms,
                    releaseDate: offer.release_date,
                    isrc: offer.isrc,
                    variant: parseVideoVariant(offer.title),
                    provider: offer.provider,
                })),
            ];
            const match = candidateSignals
                .map((candidate) => scoreVideoIdentityMatch({
                    titleA: title,
                    titleB: candidate.title,
                    lengthMsA: lengthMs,
                    lengthMsB: candidate.durationMs,
                    variantA: wantedClass,
                    variantB: candidate.variant,
                    releaseDateA: releaseDate,
                    releaseDateB: candidate.releaseDate,
                    isrcsA: exclude?.isrc,
                    isrcsB: candidate.isrc,
                    providerA: incomingProvider,
                    providerB: candidate.provider,
                    catalogIdentityB: Boolean(row.mbid || row.youtube_video_id),
                }))
    ...
    const pickPreferred = (
        ...
        const catalogRank = (row: (typeof rows)[number]) =>
            row.mbid ? 2 : row.youtube_video_id ? 1 : 0;
        const durationDistance = (row: (typeof rows)[number]) => {
            if (lengthMs == null) return Number.POSITIVE_INFINITY;
            const candidates = [
                row.length_ms,
                ...canonicalOffers(row).map((offer) => offer.duration_ms),
            ]
                .filter((value): value is number => value != null);
            if (candidates.length === 0) return Number.POSITIVE_INFINITY;
            return Math.min(...candidates.map((value) => Math.abs(Number(lengthMs) - Number(value))));
        };
        const bestCatalogRank = Math.max(...candidates.map((entry) => catalogRank(entry.row)));
        const authoritative = candidates.filter((entry) => catalogRank(entry.row) === bestCatalogRank);
        const bestDurationDistance = Math.min(...authoritative.map((entry) => durationDistance(entry.row)));
        const nearest = authoritative.filter(
            (entry) => durationDistance(entry.row) === bestDurationDistance,
        );
        // Row order is not identity evidence. Equal-authority, equal-duration
        // candidates are genuinely ambiguous and must fail closed.
        return nearest.length === 1 ? Number(nearest[0].row.id) : null;
    };

    // YouTube/MusicBrainz catalog identity: ytmusicapi can report the same
    // official cut several seconds short of Apple/TIDAL. A unique exact-title,
    // same-variant catalog row outranks provider-only rows even when those rows
    // match the provider duration more closely. Otherwise legacy provider rows
    // keep matching themselves forever and repair can never converge.
    const comparable = videoComparableTitle(title);
    const catalogHits = rows
        .filter(isEligibleCandidate)
        .filter((row) => {
            if (!row.mbid && !row.youtube_video_id) return false;
            if (!comparable || videoComparableTitle(String(row.title || "")) !== comparable) {
                return false;
            }
            if (lengthMs == null) return true;
            const durations = [
                row.length_ms,
                ...canonicalOffers(row).map((offer) => offer.duration_ms),
            ]
            ...
            return durations.some((value) => Math.abs(Number(lengthMs) - Number(value)) <= 4_000);
        })
        .map((row) => {
            const rowTitle = String(row.title || "");
            const rowVariant = coherentStoredVideoVariant(rowTitle, row.video_variant, wantedClass);
            ...
            const match = candidateSignals
                ...
                .sort((left, right) => right.score - left.score)[0];
            if (!match) return null;
            if (match.matched) return { row, match };
            if (match.reason !== "duration-hard-reject" || match.titleScore < 0.95) {
                return null;
            }
            return { row, match };
        })
    ...
    if (catalogHits.length > 0) {
        return pickPreferred(catalogHits);
    }
    ...
    if (
        ...
        const topScore = scored[0].match.score;
        const exactTitle = scored.filter(
            (entry) => topScore - entry.match.score < VIDEO_MATCH_AMBIGUITY_MARGIN
                && entry.match.titleScore >= 0.95,
        );
        return pickPreferred(exactTitle);
    }
...
function repairProviderVideoRecordingAssignments(artistMbid: string): number {
    const loadRows = () => db.prepare(`
        SELECT
            ...
            recording.youtube_video_id AS canonical_youtube_video_id
        FROM ProviderVideoMatches video_match
    ...
    `).all(artistMbid) as Array<{
        ...
        canonical_youtube_video_id: string | null;
    }>;
    ...
    let repaired = 0;
    let rows = loadRows();
    // Re-evaluate the bounded dependency graph until it settles. A wrongly
    // attached explicit variant can block its same-provider official sibling;
    // moving one edge can therefore make a later edge eligible. Canonical
    // attachments and explicit variants go first so row/provider-id order is
    // never identity evidence.
    const maxPasses = Math.min(Math.max(rows.length + 1, 2), 8);
    for (let pass = 0; pass < maxPasses && rows.length > 0; pass += 1) {
      rows.sort((left, right) => {
        const leftCanonical = left.canonical_recording_mbid || left.canonical_youtube_video_id ? 0 : 1;
        const rightCanonical = right.canonical_recording_mbid || right.canonical_youtube_video_id ? 0 : 1;
        const leftSpecific = parseVideoVariant(left.title) === "video" ? 1 : 0;
        const rightSpecific = parseVideoVariant(right.title) === "video" ? 1 : 0;
      ...
      for (const row of rows) {
        ...
        let targetRecordingId = ensureProviderVideoRecording({
            video: {
        ...
        if (
            targetRecordingId === row.recording_id
            && !row.canonical_recording_mbid
            && !row.canonical_youtube_video_id
            && parseVideoVariant(row.title) !== "video"
        ) {
            // An exact provider-only shell naturally scores above a compatible
            // cross-provider bare-title performance. Probe once without that
            // shell so explicit Live/Lyric evidence can collapse the duplicate
            // cluster instead of remaining stranded on its own row.
            targetRecordingId = findVideoRecordingIdByTitle(
                row.artist_mbid ?? artistMbid,
                String(row.title),
        ...
        // A relation belongs to the video cut, not to every provider offer that
        // happened to share its old recording. Copying all inferred audio edges
        // here attached a moved live Apple offer to the official video's studio
        // track. The relation repair below recomputes the destination from this
        // offer's own evidence and fails closed when it is ambiguous.
        ...
        const leftoverMatches = db.prepare(`
            SELECT COUNT(*) AS n
            FROM ProviderVideoMatches
            WHERE recording_id = ? AND match_state = 'accepted'
        `).get(row.recording_id) as { n: number };
        if (Number(leftoverMatches?.n || 0) === 0) {
            coalesceVideoRecordings(targetRecordingId, row.recording_id);
        }
        repaired += 1;
        passRepaired += 1;
      }
...
function repairProviderVideoAudioRelations(artistMbid: string): number {
    const rows = db.prepare(`
        SELECT
            ...
            video_match.evidence AS match_evidence
        FROM ProviderVideoMatches video_match
        JOIN ProviderItems provider_item
          ON provider_item.id = video_match.provider_video_item_id
        JOIN Recordings recording ON recording.id = video_match.recording_id
    ...
    for (const row of rows) {
        ...
        grouped.push(row);
        rowsByVideo.set(row.recording_id, grouped);
    }
    for (const providerRows of rowsByVideo.values()) {
        ...
        for (const relation of existing) {
            ...
            // Provider counterpart links are strong evidence, but the detailed
            // video title can arrive after the album row that created the link.
            // Revalidate every inferred relation so a self-OMV later identified
            // as Live cannot stay attached to a studio recording.
            const videoIsLive = isLivePerformanceTitle(videoTitle)
                || normalizeVideoVariant(videoVariant) === "live"
                || providerRows.some((candidate) =>
                    normalizeVideoVariant(candidate.video_variant) === "live");
            ...
            const liveCoreTitleCompatible = (() => {
                const videoCore = videoComparableTitle(videoTitle);
                const audioCore = videoComparableTitle(relation.audio_title);
                return Boolean(videoCore && audioCore && (
                    videoCore === audioCore
                    || videoCore.includes(audioCore)
                    || audioCore.includes(videoCore)
                ));
            })();
            const incompatibleTitle = videoIsLive && audioIsLiveSide
                ? !liveCoreTitleCompatible
                : !videoAudioTitlesCompatible(videoTitle, relation.audio_title);
        ...
        const providerMatches = providerRows.flatMap((candidate) => {
            let relatedTrackId: string | null = null;
            try {
                const evidence = JSON.parse(String(candidate.match_evidence || "{}"));
                relatedTrackId = nullableText(evidence?.relatedTrackId);
            } catch {
                relatedTrackId = null;
            }
            const audioMatch = resolveProviderVideoAudioMatch({
                video: {
                    ...
                    album_id: candidate.provider_album_id,
        ...
        const targetIds = new Set(providerMatches.map(({ audioMatch }) => audioMatch.id));
        const surviving = db.prepare(`
            SELECT target_recording_id AS audio_id, source
        ...
        providerMatches.sort((left, right) =>
...
export class RefreshVideoService {
     ...
     * Reuses the provider getVideo hook and fills NULL/empty quality. It also
     * repairs the legacy YouTube audio-tier value that album counterpart ingest
     * used to copy onto video offers. Other existing video tags are preserved.
     * Best-effort and throttled like the getVideo enrichment path.
     */
    static async backfillMissingVideoOfferQuality(
        ...
        const rows = db.prepare(`
            ...
            WHERE pi.entity_type = 'video'
              AND (
                pi.video_quality IS NULL
                OR TRIM(pi.video_quality) = ''
                OR (
                  pi.provider = 'youtube-music'
                  AND UPPER(TRIM(pi.video_quality)) = 'YOUTUBE_LOSSY'
                )
              )
              AND r.is_video = 1
              AND r.artist_mbid = ?
        ...
        const updateQuality = db.prepare(`
            ...
            WHERE provider = ?
              ...
              AND provider_id = ?
              AND (
                video_quality IS NULL
                OR TRIM(video_quality) = ''
                OR (
                  provider = 'youtube-music'
                  AND UPPER(TRIM(video_quality)) = 'YOUTUBE_LOSSY'
                )
              )
        `);
    ...
    /**
     * Persist a public-catalog video selected by the sparse-provider resolver,
     * then fold the provider-only shell into that catalog identity. The caller
     * has already required a unique title, duration, and date winner.
     */
    static upsertVerifiedCatalogVideoTwin(
        artistId: string,
        video: any,
        providerOnlyRecordingId: number,
    ): number | null {
        return this.upsertVerifiedCatalogVideoTwins(artistId, [{ video, providerOnlyRecordingId }]) > 0
            ? getAcceptedProviderVideoRecordingId(
                String(video.provider || video._provider || "youtube-music"),
                String(nullableText(video.provider_id) ?? nullableText(video.providerId)),
            )
            : null;
    }

    /**
     * Batch sparse catalog twins so provider persistence happens once and the
     * expensive whole-artist assignment/relation repair runs once, not once per
     * resolved video.
     */
    static upsertVerifiedCatalogVideoTwins(
        artistId: string,
        matches: Array<{ video: any; providerOnlyRecordingId: number }>,
    ): number {
        const artistMbid = getArtistMusicBrainzId(artistId);
        if (!artistMbid || matches.length === 0) return 0;
        const valid = matches.filter(({ providerOnlyRecordingId }) => {
        const source = db.prepare(`
            SELECT id, artist_mbid, mbid, youtube_video_id
            FROM Recordings
            WHERE id = ? AND is_video = 1
        `).get(providerOnlyRecordingId) as {
            id: number;
            artist_mbid: string | null;
            mbid: string | null;
            youtube_video_id: string | null;
            ...
            || source.artist_mbid !== artistMbid
            );
        });
        if (valid.length === 0) return 0;

        this.upsertArtistVideos(
            artistId,
            valid.map(({ video }) => video),
            { deferRepair: true },
        );

        return db.transaction(() => {
            let mergedCount = 0;
            for (const { video, providerOnlyRecordingId } of valid) {
                const provider = String(video.provider || video._provider || "youtube-music");
                const providerId = nullableText(video.provider_id) ?? nullableText(video.providerId);
                if (!providerId) continue;
                const catalogRecordingId = getAcceptedProviderVideoRecordingId(provider, providerId);
                if (!catalogRecordingId) continue;
                coalesceVideoRecordings(catalogRecordingId, providerOnlyRecordingId);
                mergedCount += 1;
            }
            repairProviderVideoRecordingAssignments(artistMbid);
            repairProviderVideoAudioRelations(artistMbid);
