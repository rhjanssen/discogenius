...
function seedAlbum(options: {
  ...
  for (const library of libraries) {
    ...
    const plan = seedSelectedAcquisitionPlan(db, { libraryEditionId: libraryRelease.id, provider: spatial ? options.spatialProvider : options.stereoProvider }) as { id: number };
