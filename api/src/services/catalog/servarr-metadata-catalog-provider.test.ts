...
function spyService(overrides: Partial<Record<string, (...args: any[]) => any>> = {}) {
...
test("getArtist delegates straight to service.getArtistInfo", async () => {
  const { service, calls } = spyService();
  const provider = new ServarrMetadataCatalogProvider(service as any);
...
test("getReleaseGroup delegates to service.getAlbumInfo", async () => {
  const { service, calls } = spyService();
  const provider = new ServarrMetadataCatalogProvider(service as any);
...
test("exposes stable id/name", () => {
  const provider = new ServarrMetadataCatalogProvider(spyService().service as any);
