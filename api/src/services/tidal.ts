 ...
 * Based on tidal-dl-ng: Retry(total=5, backoff_factor=1)
 * Schedule: 1s, 2s, 4s, 8s, 16s (total max wait: ~31s)
 */
async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const is5xx = status && parseInt(status) >= 500 && parseInt(status) < 600;

      // Don't retry on non-retryable errors
      if (!is429 && !is5xx && attempt === 1) {
        throw error;
      }
      ...
      console.log(`[Retry] ${context} failed (${status || 'error'}), retry ${attempt}/${maxRetries} in ${delaySec}s`);
...
const RATE_LIMIT_MIN_WAIT_MS = 20_000; // Tidalrr: wait 20s on 429
const RETRY_BACKOFF_FACTOR = 1.5; // old scripts: backoff factor for 5xx/429
...
const MAX_REQUEST_INTERVAL_MS = 3000;  // Maximum backoff between requests (3s)
const BACKOFF_MULTIPLIER = 1.5;        // Increase interval by 50% on 429
const RECOVERY_RATE = 0.95;            // Decrease interval by 5% on success
const REQUESTS_BEFORE_RECOVERY = 10;   // Successful requests before recovery kicks in
const RATE_LIMIT_HISTORY_SIZE = 50;    // Track last N requests for metrics
...
function recordRequest(was429: boolean) {
  const now = Date.now();
  ...
  if (was429) {
    // Got rate limited - increase interval
...
// Get current rate limit metrics for debugging
...
// Wait for both rate limit and minimum request interval
async function waitForRequestSlot(context: string) {
  const now = Date.now();

  // Wait for any active rate limit window
...
async function acquireRequestSlot(context: string) {
...
async function tidalFetchWithRetry(
  url: string,
  options: RequestInit,
  context: string,
