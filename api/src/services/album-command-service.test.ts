...
function seedAlbum() {
  ...
  db.prepare(`
    INSERT INTO AlbumReleases (Id, ForeignReleaseId, mbid, release_group_mbid, artist_mbid, title, status, track_count, media_count)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(201, "release-mbid-1", "release-mbid-1", "release-group-mbid-1", "artist-mbid-1", "Album One", "Official", 1, 1);
  db.prepare(`
    INSERT INTO Recordings (Id, ForeignRecordingId, mbid, artist_mbid, title, IsVideo)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(301, "recording-mbid-1", "recording-mbid-1", "artist-mbid-1", "Track One", 0);
  db.prepare(`
    INSERT INTO Tracks (Id, ForeignTrackId, ForeignRecordingId, mbid, release_mbid, recording_mbid, medium_position, position, number, title)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
