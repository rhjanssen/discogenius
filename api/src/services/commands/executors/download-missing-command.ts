...
import { DownloadMissingService, type DownloadMissingResult } from "../../music/download-missing-service.js";
...
export class DownloadMissingCommand implements IExecuteCommand<"DownloadMissing"> {
    private async queueForScope(
        ...
        const total: DownloadMissingResult = {
            albums: 0,
            tracks: 0,
            videos: 0,
            alreadyQueued: 0,
        ...
        if (artistIds && artistIds.length > 0) {
            ...
            for (let index = 0; index < artists.length; index += 1) {
                ...
                total.missingPlans += queued.missingPlans;
            }
            return total;
        }
    ...
    async execute(job: CommandModelOf<"DownloadMissing">, ctx: CommandHandlerContext): Promise<void> {
        ...
        const missingPlanNote = queued.missingPlans > 0
            ? `; skipped ${queued.missingPlans} edition(s) with no acquisition plan`
            : "";

        ctx.updateCommandDescription(job, {
            progress: 100,
            description: total > 0
                ? `Queued ${total} download(s) (${queued.albums} albums, ${queued.tracks} tracks, ${queued.videos} videos)${missingPlanNote}`
                : alreadyQueued > 0
                    ? `${alreadyQueued} missing album(s) already waiting in the download queue${missingPlanNote}`
                    : queued.missingPlans > 0
                        ? `Skipped ${queued.missingPlans} monitored edition(s) with no acquisition plan; run curation first`
