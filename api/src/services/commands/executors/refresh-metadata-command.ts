...
import { handleRefreshMetadata } from "../handlers/refresh-handlers.js";

/**
 * Executor for RefreshMetadata
 * Wraps the legacy handler to implement the new Lidarr-style IExecuteCommand contract.
 */
export class RefreshMetadataCommand implements IExecuteCommand<any> {
    async execute(job: any, ctx: CommandHandlerContext): Promise<void> {
        await handleRefreshMetadata(job, ctx);
    }
}
