 ...
 *
 * See docs/JOB_EXECUTION_THREADING_PLAN.md.
 */

/** Marker placed in `workerData` when *we* spawn a job worker. */
export const JOB_WORKER_MARKER = "discogeniusJobWorker" as const;
...
export type WorkerToMainMessage =
    ...
    | { kind: "importProgress"; jobId: number; state: unknown }
    | { kind: "done"; jobId: number }
 ...
 * stay inert outside the job-worker context.
 */
export function isJobWorker(): boolean {
    return !isMainThread && !!parentPort && (workerData as Record<string, unknown> | null)?.[JOB_WORKER_MARKER] === true;
}

function postToMain(message: WorkerToMainMessage): void {
    // Guarded by isJobWorker() so this is a cheap no-op on the main thread.
    if (!isJobWorker() || !parentPort) return;
 ...
 */
export function forwardImportProgress(jobId: number, state: unknown): void {
    postToMain({ kind: "importProgress", jobId, state });
}
