...
import { CommandNames, type CommandModelOf } from "../command-queue.js";
...
import { forwardImportProgress, isJobWorker, type MainToWorkerMessage, type WorkerToMainMessage } from "./job-protocol.js";
 ...
 * back to the main thread (see job-protocol.ts). We never call initDatabase()
 * here — migrations stay a main-thread, single-writer concern.
 */

if (!parentPort || !isJobWorker()) {
    throw new Error("job-worker-entry loaded outside a Discogenius job worker thread");
}
    ...
    try {
        if (job.name === CommandNames.ImportDownload) {
            ...
            if (handler) {
                ...
                console.warn(`[job-worker] picked up unhandled command: ${job.name}`);
            }
        }
        post({ kind: "done", jobId: job.id });
    } catch (error: any) {
        post({ kind: "error", jobId: job.id, message: error?.message || "Unknown job worker error" });
    }
}
    ...
    switch (message.kind) {
        case "run":
            ...
            void runJob(message).catch((error: any) => {
                post({ kind: "error", jobId: message.job.id, message: error?.message || "Unknown job worker error" });
            });
            break;
        case "shutdown":
            port.close();
