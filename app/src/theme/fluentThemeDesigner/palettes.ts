...
const hueIndexForKeyColor = (keyColor: string): number => {
    const hue = hexToHue(keyColor);
    const maxIndex = hueToSnappingPointsMap.length - 1;
    if (maxIndex <= 0) {
        return 0;
    }
    const clampedHue = Math.min(360, Math.max(0, hue));
    return Math.min(maxIndex, Math.max(0, Math.round((clampedHue / 360) * maxIndex)));
};

const snappingPointsForKeyColor = (keyColor: string): number[] => {
    const hueIndex = hueIndexForKeyColor(keyColor);
    const snappingPoints = hueToSnappingPointsMap[hueIndex] ?? hueToSnappingPointsMap[0];
    const range = [
        snappingPoints[0] * 100,
        snappingPoints[1] * 100,
        snappingPoints[2] * 100,
    ];
    return range;
};
