    ...
    const statCards = [
        {
            ...
            detail: `${formatCompactNumber(libraryStats?.artists?.downloaded)} downloaded • ${formatCompactNumber(libraryStats?.artists?.total)} in database`,
        },
        {
            key: 'albums',
            label: 'Albums',
            icon: <span className={`${styles.statIconSlot} ${styles.statIconAlbums}`}><Album24Regular className={styles.statIcon} /></span>,
            value: formatCompactNumber(libraryStats?.albums?.monitored),
            detail: `${formatCompactNumber(libraryStats?.albums?.downloaded)} downloaded • ${formatCompactNumber(libraryStats?.albums?.total)} in database`,
        },
        {
            key: 'tracks',
            label: 'Tracks',
            icon: <span className={`${styles.statIconSlot} ${styles.statIconTracks}`}><MusicNote224Regular className={styles.statIcon} /></span>,
            value: formatCompactNumber(libraryStats?.tracks?.monitored),
            detail: `${formatCompactNumber(libraryStats?.tracks?.downloaded)} downloaded • ${formatCompactNumber(libraryStats?.tracks?.total)} in database`,
        },
        {
            key: 'videos',
            label: 'Videos',
            icon: <span className={`${styles.statIconSlot} ${styles.statIconVideos}`}><Video24Regular className={styles.statIcon} /></span>,
            value: formatCompactNumber(libraryStats?.videos?.monitored),
            detail: `${formatCompactNumber(libraryStats?.videos?.downloaded)} downloaded • ${formatCompactNumber(libraryStats?.videos?.total)} in database`,
        },
    ];
